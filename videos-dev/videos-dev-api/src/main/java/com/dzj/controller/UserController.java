...
public class UserController {
	...
	@ApiOperation(value = "查询用户关注的接口", notes = "")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "userId", value = "用户id", required = true, dataType = "String", paramType = "query"),
			@ApiImplicitParam(name = "page", value = "当前第几页", required = true, dataType = "Integer", paramType = "query") })
	@PostMapping(value = "/queryUserFollow")
	public JSONResult queryUserFollow(String userId, Integer page) {
		if (page == null || page <= 0) {
			page = 1;
		}
		PageResult pageResult = userService.queryUserFollowByLimit(userId, page, ConfigClass.PAGR_SIZE);
